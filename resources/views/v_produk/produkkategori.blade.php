...
@section('content')
    ...
    @php
        $currentKategori = $kategori->firstWhere('id', request()->segment(3));
    @endphp
    <!-- CATALOG HEADER -->
    <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 20px; flex-wrap: wrap; gap: 10px;">
        <div>
            <h3 style="font-size: 22px; font-weight: 800; margin: 0; color: var(--text-main);">Kategori: {{ $currentKategori->nama_kategori ?? 'Semua Produk' }}</h3>
            <p style="font-size: 13.5px; color: var(--text-muted); margin: 0;">Menampilkan produk dalam kategori pilihan</p>
        </div>
        <div>
            <span class="badge-modern" style="background: var(--primary-light); color: var(--primary); font-size: 13px; padding: 6px 14px;">
                <i class="fa fa-tag"></i> {{ $produk->total() }} Produk
            </span>
        </div>
    </div>
    ...
    <div class="row">
        ...
        @empty
            <div class="col-12 text-center" style="padding: 40px 0;">
                ...
                <a href="{{ route('produk.all') }}" class="btn-card-detail" style="margin-top: 10px;">Lihat Semua Produk</a>
            </div>
        @endforelse
    </div>
