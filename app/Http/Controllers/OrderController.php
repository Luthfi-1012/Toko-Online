...
class OrderController extends Controller
{
    ...
    // // Kembalikan JSON dengan URL redirect
    // return response()->json([
    //     'success'  => true,
    //     'redirect' => route('order.selectpayment')
    // ]);
...
public function processPayment()
{
    ...
    // // Proses pembayaran bisa pakai gateway atau tampilkan halaman konfirmasi
    // return view('order.payment'); // Sesuaikan nama Blade-nya

    // return back()->with('error', 'Tidak ada order yang bisa diproses.');
    ...
    if ($order) {
        $order->status = 'paid';
        $order->paid_at = now();
        $order->metode_pembayaran = 'manual'; // atau 'cod', tergantung use case kamu
        $order->save();
    ...
    if ($order) { 
        foreach ($order->orderItems as $item) { 
            ...
            if ($produk->stok >= $item->quantity) { 
                ...
                return redirect()->route('order.cart')->with('error', 'Stok produk ' . 
            $produk->nama_produk . ' tidak mencukupi'); 
                }    
            } 
    ...
    public function history()
{
    ...
    $index = Order::with('customer')->get();
    return view('backend.v_order.index', compact('index'));
}
