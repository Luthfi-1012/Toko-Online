...
class UserController extends Controller
{
    ...
    public function store(Request $request)
    {
        $validatedData = $request->validate([
            ...
            'email' => 'required|max:255|email|unique:user', // Fixed the unique rule
            'role' => 'required',
            'hp' => 'required|min:10|max:13',
            'password' => 'required|min:4|confirmed', // Ensure 'password_confirmation' is present in the form
        ...
        // Using ImageHelper to handle image upload
        if ($request->file('foto')) {
            ...
            // Save the image with specified size
        ...
        // Check for lowercase ([a-z]), uppercase ([A-Z]), digits (\d), and special characters (?=.*[\W_])
        if (preg_match($pattern, $password)) {
            $validatedData['password'] = Hash::make($validatedData['password']);
            User::create($validatedData); // Removed $messages from here as it's not needed
